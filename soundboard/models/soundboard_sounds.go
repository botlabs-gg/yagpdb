...
func (o *SoundboardSound) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
...
func (o *SoundboardSound) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(soundboardSoundAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(soundboardSoundPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert soundboard_sounds, could not build conflict column list")
			}

			conflict = make([]string, len(soundboardSoundPrimaryKeyColumns))
			copy(conflict, soundboardSoundPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"soundboard_sounds\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *SoundboardSound) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return SoundboardSoundExists(ctx, exec, o.ID)
}
