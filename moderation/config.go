...
// Config is a models.ModerationConfig with correct column types (and also has
// validation tags for use with forms.)
//
// Why two seemingly similar structs, models.ModerationConfig (autogenerated by
// sqlboiler) and Config?
//
// Many of the columns in a models.ModerationConfig should really be
// non-nullable (for instance, all booleans), but these constraints were not
// applied when the table was first created and it is unfeasible to
// retroactively add them -- see comments in schema.go for more.
//
// These legacy column types create a number of annoying problems. Notably, in
// many parts of the code, we want to treat, e.g., the BanEnabled option (but
// more generally, all boolean options) as a simple yes/no switch, but since
// sqlboiler faithfully reflects the state of the database and these columns are
// marked as nullable in the schema, BanEnabled is not a bool but rather a
// null.Bool. Hence it has three states -- unset (null), set to true, and set to
// false -- rather than two. Yet since we implicitly assume that unset booleans
// are equivalent to an explicitly set false value, the additional unset state
// complicate business logic for no real benefit. Similar papercuts arise when
// we work with legacy string ID fields such as ActionChannel, which need to be
// converted to integers before use.
//
// The Config struct encapsulates all this complexity, making writing business
// code mostly palatable: its fields have the nice-to-work-with types that would
// otherwise be used if we were not burdened by legacy considerations.
// Consequently, throughout the moderation plugin we pass around and operate on
// Configs as long as possible, converting to and from models.ModerationConfigs
// (via Config.ToModel and configFromModel) only when needed.
type Config struct {
	GuildID   int64
	CreatedAt time.Time
	UpdatedAt time.Time
