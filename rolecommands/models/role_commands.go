...
func (o *RoleCommand) GetRoleGroup() *RoleGroup {
	if o == nil {
		return nil
	}

	return o.R.GetRoleGroup()
}
...
func (o *RoleCommand) GetRoleMenuOptions() RoleMenuOptionSlice {
	if o == nil {
		return nil
	}

	return o.R.GetRoleMenuOptions()
}
...
func (o *RoleCommand) GetNextRoleCommandRoleMenus() RoleMenuSlice {
	if o == nil {
		return nil
	}

	return o.R.GetNextRoleCommandRoleMenus()
}
...
func (roleCommandL) LoadRoleGroup(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleCommand interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeRoleCommand.(*RoleCommand)
		if !ok {
			object = new(RoleCommand)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeRoleCommand)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeRoleCommand))
			}
		}
	} else {
		s, ok := maybeRoleCommand.(*[]*RoleCommand)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeRoleCommand)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeRoleCommand))
			}
		}
	}
...
func (roleCommandL) LoadRoleMenuOptions(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleCommand interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`role_menu_options.role_command_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (roleCommandL) LoadNextRoleCommandRoleMenus(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleCommand interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`role_menus`),
		qm.WhereIn(`role_menus.next_role_command_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *RoleCommand) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RoleCommand) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(roleCommandAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(roleCommandPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert role_commands, could not build conflict column list")
			}

			conflict = make([]string, len(roleCommandPrimaryKeyColumns))
			copy(conflict, roleCommandPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"role_commands\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the RoleCommand row exists.
func (o *RoleCommand) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return RoleCommandExists(ctx, exec, o.ID)
}
