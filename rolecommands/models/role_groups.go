...
func (o *RoleGroup) GetRoleCommands() RoleCommandSlice {
	if o == nil {
		return nil
	}

	return o.R.GetRoleCommands()
}
...
func (o *RoleGroup) GetRoleMenus() RoleMenuSlice {
	if o == nil {
		return nil
	}
...
func (roleGroupL) LoadRoleCommands(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleGroup interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeRoleGroup.(*RoleGroup)
		if !ok {
			object = new(RoleGroup)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeRoleGroup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeRoleGroup))
			}
		}
	} else {
		s, ok := maybeRoleGroup.(*[]*RoleGroup)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeRoleGroup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeRoleGroup))
			}
		}
	}
	...
	query := NewQuery(
		...
		qm.WhereIn(`role_commands.role_group_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (roleGroupL) LoadRoleMenus(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleGroup interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`role_menus`),
		qm.WhereIn(`role_menus.role_group_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *RoleGroup) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RoleGroup) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(roleGroupAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(roleGroupPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert role_groups, could not build conflict column list")
			}

			conflict = make([]string, len(roleGroupPrimaryKeyColumns))
			copy(conflict, roleGroupPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"role_groups\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the RoleGroup row exists.
func (o *RoleGroup) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return RoleGroupExists(ctx, exec, o.ID)
}
