...
func (o *RoleMenuOption) GetRoleCommand() *RoleCommand {
	if o == nil {
		return nil
	}

	return o.R.GetRoleCommand()
}
...
func (o *RoleMenuOption) GetRoleMenu() *RoleMenu {
	if o == nil {
		return nil
	}

	return o.R.GetRoleMenu()
}
...
func (o *RoleMenuOption) GetEditingOptionRoleMenus() RoleMenuSlice {
	if o == nil {
		return nil
	}

	return o.R.GetEditingOptionRoleMenus()
}
...
func (roleMenuOptionL) LoadRoleCommand(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleMenuOption interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeRoleMenuOption.(*RoleMenuOption)
		if !ok {
			object = new(RoleMenuOption)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeRoleMenuOption)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeRoleMenuOption))
			}
		}
	} else {
		s, ok := maybeRoleMenuOption.(*[]*RoleMenuOption)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeRoleMenuOption)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeRoleMenuOption))
			}
		}
	}
	...
	if singular {
		...
		if !queries.IsNil(object.RoleCommandID) {
			args[object.RoleCommandID] = struct{}{}
		}
...
func (roleMenuOptionL) LoadRoleMenu(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleMenuOption interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`role_menus`),
		qm.WhereIn(`role_menus.message_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (roleMenuOptionL) LoadEditingOptionRoleMenus(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRoleMenuOption interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		qm.From(`role_menus`),
		qm.WhereIn(`role_menus.editing_option_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *RoleMenuOption) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RoleMenuOption) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(roleMenuOptionAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(roleMenuOptionPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert role_menu_options, could not build conflict column list")
			}

			conflict = make([]string, len(roleMenuOptionPrimaryKeyColumns))
			copy(conflict, roleMenuOptionPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"role_menu_options\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the RoleMenuOption row exists.
func (o *RoleMenuOption) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return RoleMenuOptionExists(ctx, exec, o.ID)
}
