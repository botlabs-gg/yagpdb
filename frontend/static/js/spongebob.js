...
function navigate(
  ...
  req.addEventListener("load", function () {
    ...
    if (maintainScroll) document.documentElement.scrollTop = scrollBeforeNav;
...
function updateSelectedMenuItem(pathname) {
  ...
  if (bestMatch !== -1) {
    var collapseParent =
      navLinks[bestMatch].parentElement.parentElement.parentElement;
...
function addListeners() {
  ...
  $(document)
    .on("click", ".btn-add", function (e) {
      ...
      newEntry
        .parent()
        .find(".entry:not(:last-of-type) .btn-add")
        .removeClass("btn-add")
        .addClass("btn-remove")
  ...
  $(window).on("sidebar-left-toggle", function (evt, data) {
    ...
    document.cookie =
      "sidebar_collapsed=" + data.added + "; max-age=3153600000; path=/";
  });
}
...
function channelRequirepermsDropdown(prefix) {
  ...
  dropdowns.each(function (i, rawElem) {
    trackChannelDropdown($(rawElem), [
      discordPermissions.read,
      discordPermissions.send,
    ]);
  });
...
function trackChannelDropdown(dropdown, perms) {
  var currentElem = $(
    '<p class="form-control-static">Checking channel permissions for bot...</p>'
  );
...
function validateChannelDropdown(dropdown, currentElem, channel, perms) {
  ...
  if (
    cachedChannelPerms[channel] &&
    (!cachedChannelPerms[channel].lastChecked ||
      Date.now() - cachedChannelPerms[channel].lastChecked < 5000)
  ) {
    ...
    createRequest(
      "GET",
      "/api/" + CURRENT_GUILDID + "/channelperms/" + channel,
      null,
      function () {
        console.log(this);
...
function formSubmissionEvents() {
  ...
  $(document).on("click", "button", function (event) {
    ...
    if (
      target.hasClass("btn-danger") ||
      target.attr("data-open-confirm") ||
      target.hasClass("delete-button")
    ) {
      var title = target.attr("title");
      if (title !== undefined) {
        if (
          !confirm("Deleting " + title + ". Are you sure you want to do this?")
        ) {
    ...
    event.preventDefault();
    console.log(
      "Should submit using " + target.attr("formaction"),
      event,
      parentForm
    );
...
function trackForm(form) {
  ...
  var observer = new MutationObserver(function (mutationList, observer) {
    ...
    if (isSavingUnsavedForms) checkForUnsavedChanges();
  });
...
function updateUnsavedChangesPopup() {
  if (unsavedChangesStack.length == 0) {
    ...
    if (unsavedChangesStack.length == 1) {
      ...
      $("#unsaved-changes-message").text(
        "You have unsaved changes on multiple forms, save them all?"
      );
      if (!isSavingUnsavedForms)
        $("#unsaved-changes-save-button").attr("hidden", false);
    }
