...
func (o *JoinedGuild) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
...
func (o *JoinedGuild) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(joinedGuildAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(joinedGuildPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert joined_guilds, could not build conflict column list")
			}

			conflict = make([]string, len(joinedGuildPrimaryKeyColumns))
			copy(conflict, joinedGuildPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"joined_guilds\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *JoinedGuild) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return JoinedGuildExists(ctx, exec, o.ID)
}
