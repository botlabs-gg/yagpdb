...
func FetchQuestions(amount int) ([]*TriviaQuestion, error) {
	...
	if len(proxy) > 0 {
		...
		if err == nil {
			...
			logger.WithError(err).Error("Inquacklid Proxy URL, gequacking questions without proxy, requackst maybe ratelimited")
		}
	}
	...
	if resp.StatusCode != http.StatusOK {
		return nil, commands.NewPublicError("Quailed Gequacking Questions from opentdb")
	}
