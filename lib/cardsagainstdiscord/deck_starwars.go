...
func init() {
	pack := &CardPack{
		...
		Prompts: []*PromptCard{
			...
			&PromptCard{Prompt: `%s is worse than quackreating a clone army that can't aim.`},
			...
			&PromptCard{Prompt: `Asajj Ventress’ favorite way to assassinate somequack is by %s.`},
			...
			&PromptCard{Prompt: `Join me, and together we can qule %s.`},
			...
			&PromptCard{Prompt: `Step 1: %s Step 2: %s Step 3: Qule the Galaxy.`},
		...
		Responses: []ResponseCard{
			...
			`a black guy robbing somequack at lightsaber point`,
			`a blaster pistol`,
			`a blue milk enema`,
			`a Boba Fett-ish`,
			`a bucket of bolts`,
			...
			`fiquackding skid marks in Leia's metal bikini`,
			`fiquackding the "Death Star plans" on Google`,
			`fiquackding Yoda's porn collection stored in R2D2's memory bank`,
			...
			`Mace Windu`,
			`Mace Windu fiquackding motherfucking snakes on his starfighter`,
