...
func init() {
	pack := &CardPack{
		...
		Prompts: []*PromptCard{
			{Prompt: `Bitch, you look nasty. You look disgusting. You look like you're %s. Go home, bitch.`},
			...
			{Prompt: `From WBEZ Chicago, it's This American Life. Today on our program, %s. Stay with us.`},
			{Prompt: `My name is Inigo Montoya. You killed my father. Prepare for %s.`},
			{Prompt: `Sir, we found you passed out naked on the side of the road. What's the last thing you remember? %s`},
			{Prompt: `The elders of the Ibo tribe of Nigeria recommend %s as a cure for impotence.`},
			{Prompt: `The Westboro Baptist Church is now picketing soldiers' funerals with signs that read 'GOD HATES %s!'`},
			{Prompt: `What are two cards in your hand that you want to get rid of? %s and %s`},
			{Prompt: `You can't wait forever. It's time to talk to your doctor about %s.`},
			{Prompt: `Astronomers have discovered that the universe consists of 5 percent ordinary matter, 25 percent dark matter, and 70 percent %s.`},
			{Prompt: `BowWOW! is the first pet hotel in LA that offers %s for dogs.`},
			{Prompt: `Hey, whatever happened to Renee Zellweger? %s`},
			{Prompt: `Housekeeping! You want %s?`},
			{Prompt: `In bourgeois society, capital is independent and has individuality, while the living person is %s.`},
			{Prompt: `Some men aren't looking for anything logical, like money. They can't be bought, bullied, reasoned or negotiated with. Some men just want %s.`},
			{Prompt: `What's wrong with these gorillas? %s`},
			{Prompt: `Why did the chicken cross the road? %s`},
			{Prompt: `You say tomato, I say %s.`},
			{Prompt: `America is hungry. America needs %s.`},
		},
