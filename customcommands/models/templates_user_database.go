...
func (o *TemplatesUserDatabase) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *TemplatesUserDatabase) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(templatesUserDatabaseAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(templatesUserDatabasePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert templates_user_database, could not build conflict column list")
			}

			conflict = make([]string, len(templatesUserDatabasePrimaryKeyColumns))
			copy(conflict, templatesUserDatabasePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"templates_user_database\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the TemplatesUserDatabase row exists.
func (o *TemplatesUserDatabase) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return TemplatesUserDatabaseExists(ctx, exec, o.ID)
}
