...
func (o *CustomCommandGroup) GetGroupCustomCommands() CustomCommandSlice {
	if o == nil {
		return nil
	}

	return o.R.GetGroupCustomCommands()
}
...
func (customCommandGroupL) LoadGroupCustomCommands(ctx context.Context, e boil.ContextExecutor, singular bool, maybeCustomCommandGroup interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeCustomCommandGroup.(*CustomCommandGroup)
		if !ok {
			object = new(CustomCommandGroup)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeCustomCommandGroup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeCustomCommandGroup))
			}
		}
	} else {
		s, ok := maybeCustomCommandGroup.(*[]*CustomCommandGroup)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeCustomCommandGroup)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeCustomCommandGroup))
			}
		}
	}
	...
	query := NewQuery(
		...
		qm.WhereIn(`custom_commands.group_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *CustomCommandGroup) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *CustomCommandGroup) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(customCommandGroupAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(customCommandGroupPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert custom_command_groups, could not build conflict column list")
			}

			conflict = make([]string, len(customCommandGroupPrimaryKeyColumns))
			copy(conflict, customCommandGroupPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"custom_command_groups\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the CustomCommandGroup row exists.
func (o *CustomCommandGroup) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return CustomCommandGroupExists(ctx, exec, o.ID)
}
