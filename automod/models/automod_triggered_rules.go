...
func (o *AutomodTriggeredRule) GetRule() *AutomodRule {
	if o == nil {
		return nil
	}
...
func (o *AutomodTriggeredRule) GetTrigger() *AutomodRuleDatum {
	if o == nil {
		return nil
	}
...
func (automodTriggeredRuleL) LoadRule(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodTriggeredRule interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAutomodTriggeredRule.(*AutomodTriggeredRule)
		if !ok {
			object = new(AutomodTriggeredRule)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAutomodTriggeredRule)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAutomodTriggeredRule))
			}
		}
	} else {
		s, ok := maybeAutomodTriggeredRule.(*[]*AutomodTriggeredRule)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAutomodTriggeredRule)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAutomodTriggeredRule))
			}
		}
	}
...
func (automodTriggeredRuleL) LoadTrigger(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodTriggeredRule interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`automod_rule_data.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *AutomodTriggeredRule) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *AutomodTriggeredRule) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(automodTriggeredRuleAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(automodTriggeredRulePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert automod_triggered_rules, could not build conflict column list")
			}

			conflict = make([]string, len(automodTriggeredRulePrimaryKeyColumns))
			copy(conflict, automodTriggeredRulePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"automod_triggered_rules\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the AutomodTriggeredRule row exists.
func (o *AutomodTriggeredRule) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AutomodTriggeredRuleExists(ctx, exec, o.ID)
}
