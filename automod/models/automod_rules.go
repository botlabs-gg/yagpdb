...
func (o *AutomodRule) GetRuleset() *AutomodRuleset {
	if o == nil {
		return nil
	}

	return o.R.GetRuleset()
}
...
func (o *AutomodRule) GetRuleAutomodRuleData() AutomodRuleDatumSlice {
	if o == nil {
		return nil
	}

	return o.R.GetRuleAutomodRuleData()
}
...
func (o *AutomodRule) GetRuleAutomodTriggeredRules() AutomodTriggeredRuleSlice {
	if o == nil {
		return nil
	}

	return o.R.GetRuleAutomodTriggeredRules()
}
...
func (o *AutomodRule) GetRuleAutomodViolations() AutomodViolationSlice {
	if o == nil {
		return nil
	}

	return o.R.GetRuleAutomodViolations()
}
...
func (automodRuleL) LoadRuleset(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodRule interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAutomodRule.(*AutomodRule)
		if !ok {
			object = new(AutomodRule)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAutomodRule)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAutomodRule))
			}
		}
	} else {
		s, ok := maybeAutomodRule.(*[]*AutomodRule)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAutomodRule)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAutomodRule))
			}
		}
	}
...
func (automodRuleL) LoadRuleAutomodRuleData(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodRule interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`automod_rule_data.rule_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (automodRuleL) LoadRuleAutomodTriggeredRules(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodRule interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`automod_triggered_rules.rule_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (automodRuleL) LoadRuleAutomodViolations(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodRule interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`automod_violations.rule_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *AutomodRule) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *AutomodRule) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(automodRuleAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(automodRulePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert automod_rules, could not build conflict column list")
			}

			conflict = make([]string, len(automodRulePrimaryKeyColumns))
			copy(conflict, automodRulePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"automod_rules\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the AutomodRule row exists.
func (o *AutomodRule) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AutomodRuleExists(ctx, exec, o.ID)
}
