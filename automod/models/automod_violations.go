...
func (o *AutomodViolation) GetRule() *AutomodRule {
	if o == nil {
		return nil
	}
...
func (automodViolationL) LoadRule(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodViolation interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAutomodViolation.(*AutomodViolation)
		if !ok {
			object = new(AutomodViolation)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAutomodViolation)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAutomodViolation))
			}
		}
	} else {
		s, ok := maybeAutomodViolation.(*[]*AutomodViolation)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAutomodViolation)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAutomodViolation))
			}
		}
	}
...
func (o *AutomodViolation) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *AutomodViolation) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(automodViolationAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(automodViolationPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert automod_violations, could not build conflict column list")
			}

			conflict = make([]string, len(automodViolationPrimaryKeyColumns))
			copy(conflict, automodViolationPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"automod_violations\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the AutomodViolation row exists.
func (o *AutomodViolation) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AutomodViolationExists(ctx, exec, o.ID)
}
