...
func (o *AutomodRuleDatum) GetRule() *AutomodRule {
	if o == nil {
		return nil
	}
...
func (o *AutomodRuleDatum) GetTriggerAutomodTriggeredRules() AutomodTriggeredRuleSlice {
	if o == nil {
		return nil
	}

	return o.R.GetTriggerAutomodTriggeredRules()
}
...
func (automodRuleDatumL) LoadRule(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodRuleDatum interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeAutomodRuleDatum.(*AutomodRuleDatum)
		if !ok {
			object = new(AutomodRuleDatum)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeAutomodRuleDatum)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeAutomodRuleDatum))
			}
		}
	} else {
		s, ok := maybeAutomodRuleDatum.(*[]*AutomodRuleDatum)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeAutomodRuleDatum)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeAutomodRuleDatum))
			}
		}
	}
	...
	query := NewQuery(
		...
		qm.WhereIn(`automod_rules.id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (automodRuleDatumL) LoadTriggerAutomodTriggeredRules(ctx context.Context, e boil.ContextExecutor, singular bool, maybeAutomodRuleDatum interface{}, mods queries.Applicator) error {
	...
	query := NewQuery(
		...
		qm.WhereIn(`automod_triggered_rules.trigger_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *AutomodRuleDatum) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *AutomodRuleDatum) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(automodRuleDatumAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(automodRuleDatumPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert automod_rule_data, could not build conflict column list")
			}

			conflict = make([]string, len(automodRuleDatumPrimaryKeyColumns))
			copy(conflict, automodRuleDatumPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"automod_rule_data\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the AutomodRuleDatum row exists.
func (o *AutomodRuleDatum) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return AutomodRuleDatumExists(ctx, exec, o.ID)
}
