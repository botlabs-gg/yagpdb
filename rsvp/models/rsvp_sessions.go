...
func (o *RSVPSession) GetRSVPSessionsMessageRSVPParticipants() RSVPParticipantSlice {
	if o == nil {
		return nil
	}

	return o.R.GetRSVPSessionsMessageRSVPParticipants()
}
...
func (rsvpSessionL) LoadRSVPSessionsMessageRSVPParticipants(ctx context.Context, e boil.ContextExecutor, singular bool, maybeRSVPSession interface{}, mods queries.Applicator) error {
	...
	if singular {
		var ok bool
		object, ok = maybeRSVPSession.(*RSVPSession)
		if !ok {
			object = new(RSVPSession)
			ok = queries.SetFromEmbeddedStruct(&object, &maybeRSVPSession)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", object, maybeRSVPSession))
			}
		}
	} else {
		s, ok := maybeRSVPSession.(*[]*RSVPSession)
		if ok {
			slice = *s
		} else {
			ok = queries.SetFromEmbeddedStruct(&slice, maybeRSVPSession)
			if !ok {
				return errors.New(fmt.Sprintf("failed to set %T from embedded struct %T", slice, maybeRSVPSession))
			}
		}
	}
	...
	query := NewQuery(
		...
		qm.WhereIn(`rsvp_participants.rsvp_sessions_message_id in ?`, argsSlice...),
	)
	if mods != nil {
		mods.Apply(query)
	}
...
func (o *RSVPSession) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *RSVPSession) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(rsvpSessionAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(rsvpSessionPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert rsvp_sessions, could not build conflict column list")
			}

			conflict = make([]string, len(rsvpSessionPrimaryKeyColumns))
			copy(conflict, rsvpSessionPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"rsvp_sessions\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the RSVPSession row exists.
func (o *RSVPSession) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return RSVPSessionExists(ctx, exec, o.MessageID)
}
