...
func (o *UserTimezone) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *UserTimezone) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(userTimezoneAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(userTimezonePrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert user_timezones, could not build conflict column list")
			}

			conflict = make([]string, len(userTimezonePrimaryKeyColumns))
			copy(conflict, userTimezonePrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"user_timezones\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the UserTimezone row exists.
func (o *UserTimezone) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return UserTimezoneExists(ctx, exec, o.UserID)
}
