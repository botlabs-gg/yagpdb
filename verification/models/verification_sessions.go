...
func (o *VerificationSession) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *VerificationSession) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(verificationSessionAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(verificationSessionPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert verification_sessions, could not build conflict column list")
			}

			conflict = make([]string, len(verificationSessionPrimaryKeyColumns))
			copy(conflict, verificationSessionPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"verification_sessions\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the VerificationSession row exists.
func (o *VerificationSession) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return VerificationSessionExists(ctx, exec, o.Token)
}
