...
func (o *VerificationConfig) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	return o.Upsert(ctx, boil.GetContextDB(), updateOnConflict, conflictColumns, updateColumns, insertColumns, opts...)
}

// Upsert attempts an insert using an executor, and does an update or ignore on conflict.
// See boil.Columns documentation for how to properly use updateColumns and insertColumns.
func (o *VerificationConfig) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	if o == nil {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(verificationConfigAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(verificationConfigPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert verification_configs, could not build conflict column list")
			}

			conflict = make([]string, len(verificationConfigPrimaryKeyColumns))
			copy(conflict, verificationConfigPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"verification_configs\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
// Exists checks if the VerificationConfig row exists.
func (o *VerificationConfig) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return VerificationConfigExists(ctx, exec, o.GuildID)
}
