...
var (
	...
	coreConfigColumnsWithDefault    = []string{"allowed_read_only_roles", "allowed_write_roles"}
...
func (o *CoreConfig) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(coreConfigAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(coreConfigPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert core_configs, could not build conflict column list")
			}

			conflict = make([]string, len(coreConfigPrimaryKeyColumns))
			copy(conflict, coreConfigPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"core_configs\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *CoreConfig) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return CoreConfigExists(ctx, exec, o.GuildID)
}
