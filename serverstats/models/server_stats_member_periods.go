...
var ServerStatsMemberPeriodTableColumns = struct {
	ID         string
	GuildID    string
	CreatedAt  string
	NumMembers string
	...
	ID:         "server_stats_member_periods.id",
	GuildID:    "server_stats_member_periods.guild_id",
	CreatedAt:  "server_stats_member_periods.created_at",
	NumMembers: "server_stats_member_periods.num_members",
	Joins:      "server_stats_member_periods.joins",
	Leaves:     "server_stats_member_periods.leaves",
	MaxOnline:  "server_stats_member_periods.max_online",
}
...
var (
	...
	serverStatsMemberPeriodGeneratedColumns      = []string{}
)

type (
	// ServerStatsMemberPeriodSlice is an alias for a slice of pointers to ServerStatsMemberPeriod.
	// This should almost always be used instead of []ServerStatsMemberPeriod.
...
// CountG returns the count of all ServerStatsMemberPeriod records in the query using the global executor
...
func ServerStatsMemberPeriods(mods ...qm.QueryMod) serverStatsMemberPeriodQuery {
	...
	if len(queries.GetSelect(q)) == 0 {
		queries.SetSelect(q, []string{"\"server_stats_member_periods\".*"})
	}

	return serverStatsMemberPeriodQuery{q}
}
...
func (o *ServerStatsMemberPeriod) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
...
func (o *ServerStatsMemberPeriod) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(serverStatsMemberPeriodAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(serverStatsMemberPeriodPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert server_stats_member_periods, could not build conflict column list")
			}

			conflict = make([]string, len(serverStatsMemberPeriodPrimaryKeyColumns))
			copy(conflict, serverStatsMemberPeriodPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"server_stats_member_periods\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *ServerStatsMemberPeriod) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ServerStatsMemberPeriodExists(ctx, exec, o.ID)
}
