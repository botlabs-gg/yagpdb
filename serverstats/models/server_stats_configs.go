...
func (o *ServerStatsConfig) UpsertG(ctx context.Context, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
...
func (o *ServerStatsConfig) Upsert(ctx context.Context, exec boil.ContextExecutor, updateOnConflict bool, conflictColumns []string, updateColumns, insertColumns boil.Columns, opts ...UpsertOptionFunc) error {
	...
	if !cached {
		...
		ret := strmangle.SetComplement(serverStatsConfigAllColumns, strmangle.SetIntersect(insert, update))

		conflict := conflictColumns
		if len(conflict) == 0 && updateOnConflict && len(update) != 0 {
			if len(serverStatsConfigPrimaryKeyColumns) == 0 {
				return errors.New("models: unable to upsert server_stats_configs, could not build conflict column list")
			}

			conflict = make([]string, len(serverStatsConfigPrimaryKeyColumns))
			copy(conflict, serverStatsConfigPrimaryKeyColumns)
		}
		cache.query = buildUpsertQueryPostgres(dialect, "\"server_stats_configs\"", updateOnConflict, ret, update, conflict, insert, opts...)
...
func (o *ServerStatsConfig) Exists(ctx context.Context, exec boil.ContextExecutor) (bool, error) {
	return ServerStatsConfigExists(ctx, exec, o.GuildID)
}
